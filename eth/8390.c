//
// Migrated from MINIX/drivers/dpeth/8390.c
//

#include "minix-compatible.h"
void
panic_(char* who, char* mess, int num) {
  // ignore third argument 'num'
  cprintf("[minix-compatible] PM panic (%s): %s", who, mess);
  panic("[minix-compatible] ethernet error");
}
#define NO_NUM 0
#define panic panic_
#include "dp.h"
...
// shin: NE1k/2k has a programmable I/O, so mem_*() don't need.
#if 0
...
#endif
#define sys_nic2mem(srcOffs,dstProc,dstOffs,length) // nothing
#define sys_user2nic(srcProc,srcOffs,dstOffs,length) // nothing
#define sys_vircopy(a, b, c, d, e, f, g) // nothing
...
*/
static void
ns_rw_setup(dpeth_t *dep, int mode, int size, u16_t offset)
{
...
*/
static void
mem_getblock(dpeth_t *dep, u16_t offset, int size, void *dst)
{
...
*/
static void
mem_nic2user(dpeth_t * dep, int pageno, int pktsize)
{
...
*/
static void
mem_user2nic(dpeth_t *dep, int pageno, int pktsize)
{
  phys_bytes offset, phys_user;
...
*/
static void
pio_getblock(dpeth_t *dep, u16_t offset, int size, void *dst)
{
...
*/
static void
pio_nic2user(dpeth_t *dep, int pageno, int pktsize)
{
  //phys_bytes phys_user; // shin: unused
  iovec_dat_t *iovp = &dep->de_read_iovec;
  ...
  do {                          /* Reads chuncks of packet into user area */
    ...
    if ((offset + bytes) > (dep->de_stoppage * DP_PAGESIZE)) {
      ...
      insb(dep->de_data_port, (void*)(iovp->iod_iovec[ix].iov_addr), bytes);
      pktsize -= bytes;
      iovp->iod_iovec[ix].iov_addr += bytes;
      bytes = iovp->iod_iovec[ix].iov_size - bytes;
      if (bytes > pktsize) bytes = pktsize;
...
*/
static void
pio_user2nic(dpeth_t *dep, int pageno, int pktsize)
{
  //phys_bytes phys_user; // shin: unused
  iovec_dat_t *iovp = &dep->de_write_iovec;
  int bytes, ix = 0;
  ...
  do {                          /* Reads chuncks of packet from user area */
    ...
    outsb(dep->de_data_port, (void*)(iovp->iod_iovec[ix].iov_addr), bytes);
...
*/
static void
ns_send(dpeth_t * dep, int from_int, int size)
{
  int queue;
...
*/
static void
ns_recv(dpeth_t *dep, int fromint, int size)
{
  dp_rcvhdr_t header;
  //dp_rcvhdr_t dummy; // shin: unused
...
*/
static void
ns_interrupt(dpeth_t * dep)
{
  int isr, tsr;
  //int size, queue; // shin: unused
  int queue;
...
*/
static void
dp_pio16_user2nic(dpeth_t *dep, int pageno, int pktsize)
{
  u8_t two_bytes[2];
...
*/
static void
dp_pio16_nic2user(dpeth_t * dep, int nic_addr, int count)
{
