//
// Migrated from MINIX/drivers/dpeth/ne.c
//
...
ne_init(dpeth_t * dep)
{
  ...
  //dep->de_irq &= NOT(DEI_DEFAULT); // shin: unused

  ns_init(dep);                 /* Initialize DP controller */

  printf("%s: NE%d000 (%dkB RAM) at 0x%x:%d - ",
         dep->de_name,
         dep->de_16bit ? 2 : 1,
         dep->de_ramsize / 1024,
         dep->de_base_port, dep->de_irq);
  for (ix = 0; ix < SA_ADDR_LEN; ix += 1)
    printf("%x%s", dep->de_address.ea_addr[ix], ix < SA_ADDR_LEN - 1 ? ":" : "\n");
  return;
}
...
ne_probe(dpeth_t * dep)
{
  ...
  if ((loc1 = inb_ne(dep, NE_DP8390)) == 0xFF)
    return FALSE;
